...
\begin{enumerate}
  ...
  \item What preservation properties do these functors have? By preservation properties we understand properties that are dependent on geometric conditions, and imply certain preservations of subcategories of the different categories of sheaves. This is addressed in \cref{section:geometric-properties}.
\end{enumerate}
...
\begin{definition}
Let $f \colon X \to Y$ be a morphism of topological spaces and let $Z$ be a closed subset of $X$. The \emph{direct image with support in $Z$} $f_{Z,*} \colon \Sh_X \to \Sh_Y$ is defined by sending $\mathcal F \in \Ob(\Sh_X))$ to the presheaf
	\begin{equation}
		f_{Z,*} \mathcal F \colon U \mapsto \{s \in \mathcal F(f^{-1}(U)) \mid \text{supp}(s) \subset Z\}
	\end{equation}
This presheaf is already a sheaf.
\end{definition}

\begin{definition}
 Let $f \colon X \to Y$ be a morphism of locally compact topological spaces\checkthis{My reference (see below) gives the definition; I'm not sure whether the locally compactness is necessary or not}. The \emph{direct image with compact support} $f_! \colon \Sh_X \to \Sh_Y$ is defined by sending $\mathcal F \in \Ob(\Sh_Y)$ to the sheafification of
 \begin{equation}
   f_! \mathcal F \colon U \mapsto \{s \in \mathcal F(f^{-1}(U)) \mid f |_{\text{supp}(s)} \colon \text{supp}(s) \to Y \text{ is proper} \}
 \end{equation}
This presheaf is already a sheaf.\addreference{I checked the topological case on Iversen, \emph{Cohomology of sheaves}, Springer Verlag (1986), Proposition VII.1.2, but I don't know how do you want to write the references.}
\end{definition}

% for some reason this gives an error
%\checkthis{I guess that we should have a relation like $f_! = \varinjlim_Z f_{Z,*}$, where the direct limit is taken over proper sets for the map $f$. This conjectural relation -- I don't have any reference for the moment -- should lead an interesting Grothendieck spectral sequence; in some special case we could even be able to compute the lowest terms!}
...
\begin{sidewaystable}
  ...
  \begin{tabular}{cccccccc}
    \toprule
    functor & condition on the morphism & condition on~$X$ & condition on~$Y$ & locally free & coherent & quasicoherent & \\\midrule
    $f_*$ & proper & & locally noetherian & & yes \stackscite{02OS} \\
    $f_*$ & quasi-separated quasi-compact & & & & & yes \stackscite{01XJ} \\
    \bottomrule
  \end{tabular}
  \caption{Preservation properties of functors}
  \label{table:preservation-properties}
\end{sidewaystable}
